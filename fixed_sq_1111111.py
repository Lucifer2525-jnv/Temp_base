...
class SQLiteMemory(BaseMemory):
    ...
    def __init__(self, session_id: str, db_path: str = "memory.db"):
        # Make session_id accessible as a public attribute
        self.session_id = session_id
        self.db_path = db_path
        self.lock = threading.Lock()
        
        # Initialize chat_memory as empty list for agent compatibility
        self.chat_memory = []
    ...
    def _load_chat_memory(self):
        """Load existing chat history into memory for agent compatibility."""
        try:
            ...
            with self.lock:
                with self._get_connection() as conn:
                    ...
                    if user_msg:
                        ...
                        messages_to_add.append(HumanMessage(content=user_msg))
                    ...
                    if ai_msg:
                        ...
                        messages_to_add.append(AIMessage(content=ai_msg))
                    
                    conn.commit()
                    
                    # Update in-memory chat_memory for agent compatibility
                    self.chat_memory.extend(messages_to_add)
    ...
    # Additional methods that some agents might expect
    @property
    def buffer(self) -> str:
        """Return chat history as a formatted string."""
        try:
            messages = self.chat_memory or self._get_history()
            buffer_parts = []
            for msg in messages:
                if isinstance(msg, HumanMessage):
                    buffer_parts.append(f"Human: {msg.content}")
                elif isinstance(msg, AIMessage):
                    buffer_parts.append(f"AI: {msg.content}")
            return "\n".join(buffer_parts)
        except Exception as e:
    ...
    def add_user_message(self, message: str) -> None:
        ...
        try:
            with self.lock:
                ...
                # Update in-memory chat_memory
                self.chat_memory.append(HumanMessage(content=message))
                
            logger.info(f"Added user message for session: {self.session_id}")
        except Exception as e:
            logger.error(f"Error adding user message: {e}")
            raise

    def add_ai_message(self, message: str) -> None:
        """Add an AI message directly."""
        try:
            with self.lock:
                with self._get_connection() as conn:
                    conn.execute(
                        "INSERT INTO chat_memory (session_id, role, message) VALUES (?, ?, ?)", 
                        (self.session_id, "ai", message)
                    )
                    conn.commit()
            ...
            logger.info(f"Added AI message for session: {self.session_id}")
        except Exception as e:
