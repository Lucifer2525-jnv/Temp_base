...
# llm_client = create_chat_openai_client()  # Commented out - replace with your actual client
...
class RateLimiter:
    def __init__(self):
        ...
        self._lock = threading.Lock()  # Keep threading.Lock for sync operations
...
class SessionManager:
    def __init__(self):
        ...
        self._lock = asyncio.Lock()  # Use only asyncio.Lock
    ...
    def _create_agent_executor(self, memory: ConversationBufferMemory) -> AgentExecutor:
        ...
        # llm = llm_client  # Replace with your actual LLM client
        llm = ChatOpenAI(model="gpt-4")  # Placeholder - replace with your client
...
Thought: {agent_scratchpad}"""
    ...
    async def cleanup_expired_sessions(self):
        """Remove expired sessions - converted to async"""
        async with self._lock:
            now = datetime.now()
            expired_sessions = [
...
async def cleanup_sessions_task():
    ...
    while True:
        try:
            await session_manager.cleanup_expired_sessions()  # Made async
...
async def process_chat_request(request_data: Dict[str, Any]) -> ChatResponse:
    ...
    try:
        ...
        await connection_pool.release()  # Fixed: added await
...
if __name__ == "__main__":
    uvicorn.run(
        "main:app",  # Fixed: changed from "Session_Handling:app"
        host="0.0.0.0",
        port=8000,
